...
class WorldgenService {
    public async generateWorld(): Promise<GameWorld> {
        ...
        for (let i = 0; i < TOTAL_MAPS; i++) {
            ...
            const map = await this.generateMap(names, pointNames);
            maps.push(map);
        ...
        let charCount = 0;
        for (let map of maps) {
            charCount += map.characters.length;
        }
    ...
    public async generateMap(names: Set<string>, pointNames: Set<string>): Promise<GameMap> {
        ...
        for (let i = 0; i < layout.points.length; i++) {
            ...
            for (let j = 0; j < count; j++) {
                ...
                const character = await chargenService.generateCharacter(race, gender);
