...
class GameStore {
    ...
    public enterLocation() {
        ...
        this.currentMap.next(some(this.world.maps[mapIndex]));
        const [x, y] = this.world.maps[mapIndex].layout.points[0];
        this.position.next({mapIndex, pointIndex: 0, x, y});
        this.pushNarrativeEvent({type: "enter", mapName: this.world.maps[mapIndex].name});
