...
import {GameMap, OverworldMap} from "../../services/worldgen.service.ts";
...
export const WorldMap: FC<Props> = ({gameMap, pan, startPan}) => {
    const {points: mapPoints, edges: mapEdges, bridges, ap} = gameMap.layout;
    const playerPosition = useStoreSubscribe(gameStore.position);
    const [w, h] = gameMap.size;

    const enterLocation = (pointIndex: number) => {
        ...
        gameStore.enterLocation(pointIndex);
    }

    return (
        <div className="w-full h-full overflow-hidden relative">
            <h1 className="text-2xl text-center">Lucanis</h1>
                        ...
                        mapPoints.map((point, index) => {
                            return (
                                <circle
                                    ...
                                    onClick={() => {enterLocation(index)}}
                                    className={clsx("fill-current text-primary transform transition-transform duration-200 ease-in-out hover:scale-150",
                                        `point-${index}`
                                    )}
                                    style={{
            ...
            </div>
            {
                mapPoints.map((point, index) => {
                    ...
                    return (
                        <Tooltip anchorSelect={`.point-${index}`} key={`${point[0]}-${point[1]}-lucanis`}>
