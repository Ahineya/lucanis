...
class EffectService {
    ...
    public executeCondition({condition}: ConditionDSL, npc: Character) {
        ...
        for (const step of steps) {
            match(step, {
                ...
                "get_npc": () => {
                    ...
                    combine([key]).fold(([key]) => {
                        stack.push(npc[key as string]);
                    }, () => {
                        throw new Error("Not enough arguments for get_npc");
                    });
                },
                "swp": () => {
                    const a = stack.pop();
